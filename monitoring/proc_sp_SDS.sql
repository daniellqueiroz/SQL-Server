--run this to get an overview of the allocated vs free db space

ALTER PROCEDURE dbo.sp_SDS
  @TargetDatabase nvarchar(1000) = NULL,  --  NULL: all dbs  
  @Level varchar(5000) = 'Database',    --  or "File"      
  @UpdateUsage bit = 0,               --  default no update      
  @Unit char(2) = 'MB' ,               --  Megabytes, Kilobytes or Gigabytes      
  @Option nvarchar(8) = 'Detail'   --  Level of detail visualization      
AS      
      
/**************************************************************************************************      
**      
**  author: Richard Ding      
**  coauthor: Rafael Bahia      
**  date:   4/8/2008      
...
**             sp_SDS 'Test', 'File', 0, 'kb', 'Summary'      
**             sp_SDS 'pfaids', 'Database', 0, 'gb', 'detail'      
**             sp_SDS 'tempdb', NULL, 1, 'kb', 'ALL'      
**  Change History: Added parametrization for the detail level of information. See examples above.       
**************************************************************************************************/      
...
CREATE TABLE dbo.##Tbl_CombinedInfo (      
  ...
  LogicalName VARCHAR(5000) NULL,      
  T dec(10, 2) NULL,      
  U dec(10, 2) NULL,      
  [U(%)] dec(5, 2) NULL,      
  F dec(10, 2) NULL,      
  [F(%)] dec(5, 2) NULL,      
  PhysicalName VARCHAR(5000) NULL );      
...
DECLARE @Ver varchar(5000),       
        @DatabaseName varchar(5000),       
        @Ident_last int,       
        @String varchar(5000),      
        @BaseString varchar(5000);      
...
SELECT @String = 'INSERT INTO dbo.##Tbl_ValidDbs SELECT name FROM ' +       
                 CASE WHEN @Ver = 'SQL 2000' THEN 'master.dbo.sysdatabases'       
                      WHEN @Ver IN ('SQL 2005', 'SQL 2008', 'SQL 2008 R2') THEN 'master.sys.databases'         
...
IF UPPER(ISNULL(@Level, 'DATABASE')) = 'FILE'      
 ...
 IF LTRIM(RTRIM(UPPER(@Option))) = 'ALL' or LTRIM(RTRIM(UPPER(@Option))) = 'DETAIL'      
 ...
 IF LTRIM(RTRIM(UPPER(@Option))) = 'Summary' or LTRIM(RTRIM(UPPER(@Option))) = 'All'      
...
IF UPPER(ISNULL(@Level, 'DATABASE')) = 'DATABASE'      
 ...
 IF @TargetDatabase IS NULL AND (LTRIM(RTRIM(UPPER(@Option))) = 'Summary' or LTRIM(RTRIM(UPPER(@Option))) = 'All')      
      SELECT CASE WHEN @Unit = 'GB' THEN 'GB' WHEN @Unit = 'KB' THEN 'KB' ELSE 'MB' END AS 'SUM',       
      SUM (used) AS 'USED',       
      SUM (free) AS 'FREE',       
      SUM (TOTAL) AS 'TOTAL',       
