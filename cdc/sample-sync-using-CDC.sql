--This is a sample script that shows how you can use CDC to synchronize data between environments
--This solution calls a procedure installed at the data source via linked server and inserts this data into the auxiliary table
--Then, using the Merge command, data is synchronized at the destination
--This solution works well when you need data updated close  to real time (with 10-30 minutes  delay)
--THe solution also uses OPENQUERY to update the extraction control table located at the data source


--create at destination. This is an auxiliary table to perform the synchronization 
...
--create and perform the initial population at destination
