...
1. Use this script to create a procedure that will generate the create script for a particular trigger
2. Usage: SQL Server 2005 onwards
*/

if object_id('dbo.RetornaTextoTrigger') is not null
	drop PROCEDURE dbo.RetornaTextoTrigger
GO
create procedure dbo.RetornaTextoTrigger
...
select @cmd=(
select convert(nvarchar(max),Text) from #CommentText order by LineId
for xml path('')
)
