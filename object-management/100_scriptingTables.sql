...
1. Use this script to create a procedure that will generate the create script for a particular table
2. Usage: SQL Server 2005 onwards 

*/

if object_id('[dbo].[criaindices]') is not null
	drop PROCEDURE [dbo].[criaindices]
GO
CREATE procedure [dbo].[criaindices]
(@nome varchar(max))
as
set nocount on
DECLARE @indices TABLE (nome varchar(max))
SET @nome = @nome + ','
...
DECLARE @IxID INT
declare @IXincludeSQL NVARCHAR(max)
...
BEGIN
   ...
   CLOSE cIxColumn
   DEALLOCATE cIxColumn
	   SET @IXSQL = @IXSQL + ') with (fillfactor = '+CASE CAST(@fillfactor AS VARCHAR(max)) WHEN 0 THEN '100' ELSE CAST(@fillfactor AS VARCHAR(max)) END +')'
	   end
...
DECLARE @ShowFields TABLE     (         FieldID INT IDENTITY(1,1),         DatabaseName VARCHAR(max),         TableOwner VARCHAR(max),         TableName VARCHAR(100),         FieldName VARCHAR(100),         ColumnPosition INT,         ColumnDefaultValue VARCHAR(100),         ColumnDefaultName VARCHAR(100),         IsNullable BIT,         DataType VARCHAR(100),         MaxLength INT,         NumericPrecision INT,         NumericScale INT,         DomainName VARCHAR(100),         FieldListingName VARCHAR(110),         FieldDefinition CHAR(1),         IdentityColumn BIT,         IdentitySeed INT,         IdentityIncrement INT,         IsCharColumn BIT     )
DECLARE @HoldingArea TABLE     (         FldID SMALLINT IDENTITY(1,1),         Flds VARCHAR(max),         FldValue CHAR(1) DEFAULT(0)     )
DECLARE @PKObjectID TABLE     (         ObjectID INT     )     DECLARE @Uniques TABLE     (         ObjectID INT     )
DECLARE @HoldingAreaValues TABLE     (         FldID SMALLINT IDENTITY(1,1),         Flds VARCHAR(max),         FldValue CHAR(1) DEFAULT(0)     )
DECLARE @Definition TABLE     (         DefinitionID SMALLINT IDENTITY(1,1),         FieldValue VARCHAR(max)     )
...
INSERT INTO @Definition(FieldValue)     VALUES('if object_id('''+@tablename+''') is null ' + CHAR(10) + ' CREATE TABLE ' + CASE WHEN @NewTableName IS NOT NULL THEN @NewTableName ELSE + @TableSchema + '.' + @TableName END)
...
SELECT      CHAR(10) + FieldName + ' ' +
			...
			CASE WHEN IsCharColumn = 1 and DataType <> 'text' THEN '(' + replace(CAST(MaxLength AS VARCHAR(max)),'-1','max') + ')' ELSE '' END +
			CASE WHEN IsCharColumn = 1 and DataType = 'text' THEN '' END +
			CASE WHEN IdentityColumn = 1 THEN ' IDENTITY(' + CAST(IdentitySeed AS VARCHAR(max))+ ',' + CAST(IdentityIncrement AS VARCHAR(max)) + ')' ELSE '' END +
...
INSERT INTO @Definition(FieldValue)         VALUES(')' + char(10) + 'GO' + char(10))
...
SELECT @out = 
(
	SELECT convert(nvarchar(max),FieldValue) FROM @MainDefinition
	FOR XML PATH('')
)
...
--declare @outp xml; exec sp_ScriptTable 'administradorcampanha',@includeindexes=1,@out=@outp output; select @outp
