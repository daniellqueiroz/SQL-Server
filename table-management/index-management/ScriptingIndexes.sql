-- use this script to get all the indexes inside the database. You can also list all the indexes of a specific table
...
(
	...
	(
		...
		--and user_seeks = 0 AND user_lookups = 0 AND user_scans = 0 -- uncomment here to get totally obsolete indexes
		AND o.is_ms_shipped=0
		--AND user_updates > 0
		--AND is_primary_key = 0
		--AND si.is_unique=0
		--AND user_updates < 5000
		--AND si.name = 'IX_CompraStatusLog_IdCompra' -- put the name of specific indexes here
		and OBJECT_NAME(si.object_id) = 'COmpra' -- comment this filter to get indexes on the database
	)AS a
	WHERE 1=1
	--AND (
	--		(a.CamposChave like '%IdFabricante%' OR a.CamposChave like '%IdFornecedor%' )
	--		OR 
