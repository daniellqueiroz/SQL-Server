...
DECLARE @publicacao VARCHAR(200)='marketplace_extra',@servidor VARCHAR(100),@banco VARCHAR(50)=DB_NAME();
...
' l on c.' + (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo asc) + '=l.'+ (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo asc) + ' and c.' + (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo desc) + '=l.'+ (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo desc) + ' where (isnull('+ CASE WHEN sc.system_type_id IN (35,99) THEN 'cast(c.'+sc.NAME+ ' as varchar(max)) ' ELSE 'c.'+sc.NAME END +','+ 
...
' l on c.' + (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo asc) + '=l.'+ (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo asc) + ' and c.' + (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo desc) + '=l.'+ (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo desc) + ' where isnull(c.'+sc.NAME+ ','+ 
...
' c on c.' + (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo asc) + '=l.'+ (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo asc) + ' and c.' + (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo desc) + '=l.'+ (SELECT top 1 campo FROM #chaves WHERE tabela=a.name order by campo desc) + ' where (isnull('+ 
...
AND a.NAME+sc.name NOT IN ('Produto_TextoMarca','Produto_TextoCategoria','Produto_TextoDepartamento','ProdutoCampoValorTexto','ProdutoDataEdicao','SkuDataCadastro')
